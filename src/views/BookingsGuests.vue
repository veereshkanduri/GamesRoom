<template>
    <div class="listings">
                ...
                <h6 v-if="!listing_duplicate.length" style="margin-top:50px; margin-bottom:30px">There are no results</h6>
    </div>
...
export default {
  ...
  methods: {
    getBookings: function() {
      ...
      if(isHost())
      axios
        .post(`${process.env.VUE_APP_BASE_URL}/need_host_bookings`, decryptUser())
        .then(response => {
          this.loading = false;
      ...
      axios
        .post(`${process.env.VUE_APP_BASE_URL}/need_previous_bookings`, decryptUser())
        .then(response => {
          this.loading = false;
          this.listing_duplicate = response.data;
        })
